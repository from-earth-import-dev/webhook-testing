...
# List to store triggered events
triggered_events: List[WebhookPayload] = []
...
def register_routes(app: Flask) -> None:
    ...
    def webhook() -> Response:
        ...
        try:
            payload: WebhookPayload = WebhookPayload(**data)
...
def trigger_alert(payload: WebhookPayload, target_url: str) -> requests.Response:
    ...
    response: requests.Response = requests.post(
        target_url, json=payload.model_dump(mode="json")
    )
    return response
